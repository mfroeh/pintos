...
start_process(void *aux)
{
  ...
  child_list_item->was_waited_on = false;
...
int process_wait(tid_t child_tid UNUSED)
{
  ...
  for (it = list_begin(&t->pcb.children); it != list_end(&t->pcb.children); it = list_next(it))
  {
    ...
    if (ch->tid == child_tid)
    {
      ...
      ASSERT(ch->is_dead);
      free(t->pcb.sema_wait);
      // printf("%d process child exit with code %d\n", thread_current()->tid, ch->exit_code);
      return ch->exit_code;
    }
  }

  // If we get here, there was no child with that tid
  return -1;
}
...
bool load(spawn_params *params, void (**eip)(void), void **esp)
{
    ...
    case PT_LOAD:
      if (validate_segment(&phdr, file))
      {
        ...
        if (phdr.p_filesz > 0)
        {
          ...
          zero_bytes = (ROUND_UP(page_offset + phdr.p_memsz, PGSIZE) - read_bytes - page_offset);
        }
        else
        {
          /* Entirely zero.
